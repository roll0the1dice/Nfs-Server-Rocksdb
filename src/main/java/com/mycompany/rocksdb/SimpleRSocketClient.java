...
//            byte[] metadataBytes = metadata.getBytes();
     ...
     * 上传元数据（请求-响应模式）
     */
    public Mono<String> putRedisdata(String metadata) {
        return rsocket.requestResponse(
                DefaultPayload.create(metadata.getBytes(), SimpleRSocketServer.PayloadMetaType.PUT_REDIS_DATA.name().getBytes())
        ).map(payload -> payload.getDataUtf8());
    }
